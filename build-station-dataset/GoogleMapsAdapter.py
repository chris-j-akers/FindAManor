...
class GoogleMapsAdapter:
    ...
    def get_journey_time(self, origin_place : Place , destination_place : Place):
        ...
        self._logger.debug(f'calling {URL.format(origin_place.name, destination_place.name)}')
        response_json = requests.get(URL.format(f'place_id:{origin_place.place_id}', f'place_id:{destination_place.place_id}')).json()
        if (response_json['rows'][0]['elements'][0]['status'] == 'OK'):
            self._logger.debug(f'got value for [{destination_place.name}], returning [{response_json['rows'][0]['elements'][0]['duration']['value']}]')
            self._logger.debug(f'{self._format_json(response_json)}')            
            return response_json['rows'][0]['elements'][0]['duration']['value']
        else:
            self._logger.debug(f'no value for [{destination_place.name}]')
            print(f'destination: [{destination_place.name}] not found or no specific match')
            return None
